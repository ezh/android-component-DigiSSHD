...
object AllowAdapter extends Logging {
  private[session] lazy val adapter: Option[Adapter] = AppComponent.Context map {
    context =>
      Some(new Adapter(context))
  } getOrElse { log.fatal("unable to create AllowAdapter"); None }
  log.debug("alive")
